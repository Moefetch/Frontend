<template>
    <div class="h-[1.84rem] w-[14rem]" ref="target">
        <div :class="`flex mt-auto mb-auto flex-row bg-dark-500 w-[14rem] h-[1.84rem] pl-[0.25rem] pr-[0.25rem] rounded-[0.2rem] cursor-pointer items-center ${toggled? 'rounded-b-none' : '' }`" @click="shittyClick()">
            <BaseDropMenuItem :item="currentItem" :class="`h-[1.25] w-full` "  id="currentlyDisplayed" />
    ...
    <BaseDropMenuItem v-for="item in dropdownItemsArray" :item="item" class="h-[1.25] w-[14rem] mt-[0.25rem] mb-[0.25rem] cursor-pointer" @click="clickedType(item)"/>
        </div>
    </div>
</template>
...
const props = defineProps<{dropdownItemsArray: string[]}>();
