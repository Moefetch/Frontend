  ...
  <div class="add_new_popup_container">
    <div class="flex flex-col gap-2">
      <div class="m-auto w-full justify-center align-middle items-center flex flex-col">
        <div class="overflow-hidden rounded-[0.25rem] max-w-[180px] max-h-[140px] min-h-2">
          <div v-if="picForm.tempFileStore?.length" class="flex flex-row gap-1 overflow-x-scroll">
            <div v-for="pic in picForm.tempFileStore"
              class="border-transparent border-b-width-[10px] border-t-width-[10px]">
              <img class="max-w-[90px] max-h-[100px] rounded-[8px] relative object-contain" :src="pic.filePreviewURL"
                alt="">
            </div>
          </div>
        </div>
        <div v-if="picFormArrayLength > 1" class="m-auto w-full justify-center align-middle items-center flex flex-row">
          <Button text="" icon="left" color="" :disabled="(entryIndexer == 0 && picFormArrayLength == 1)"
            class="w-[36px] rounded-[8px]" @click="changeIndexer('minus')" />

          <h2>{{ (entryIndexer + 1) + " / " + picFormArrayLength }}</h2>

          <Button text="" icon="right" :disabled="((entryIndexer - 1) == picFormArrayLength)" color=""
            class="w-[36px] rounded-[8px]" @click="changeIndexer('plus')" />
        </div>
      </div>
      ...
      <div class="flex flex-row gap-1">
        ...
        <img class="w-[28px]" src="/icons/image_add.svg" alt="" @click="picHTMLElement?.click()">
        <input type="file" multiple style="display: none;" ref="picHTMLElement" @change="onAddFiles">
      </div>

      <BaseDropMenu :dropdownItemsArray="modelTypesArray" @item-selected="typeSelect"
        :defaultSelected="defaultSelectedAlbumType" :class="`${picFormError.picTypeError ? 'error' : ''} box-content`"
      ...
      <FieldErrorSlot :errorMessage="AlbumNameErrorMessage">
        <input v-if="picForm.createAlbumToggle" type="text" v-model="picForm.createAlbumName" ref="newAlbumInput"
          id="newAlbumInput" name="newAlbumInput" style="height: 32px" class="inputField"
          placeholder="Enter new Album name" min="1" max="64" />
      </FieldErrorSlot>

      <div class="flex flex-row gap-2 pl-1" @click="advancedOptionsToggle = !advancedOptionsToggle">
        <h2>Advanced options</h2>
        <div class="w-[16px] ">
          <Icon :icon="advancedOptionsToggle ? 'up' : 'down'" />
        </div>
      </div>
      ...
      <div class="flex flex-row gap-2 pl-1" @click="advancedParamsToggle = !advancedParamsToggle">
        ...
        <div class="w-[16px] ">
          <Icon :icon="advancedParamsToggle ? 'up' : 'down'" />
        </div>
      </div>

      <div v-if="picForm.optionalOverrideParams && advancedParamsToggle">

        <div v-for="(value, key) in picForm.optionalOverrideParams">
          <div class="flex flex-col gap-[0.5rem]">
      ...
      <Button text="Submit" color="#4d6d8d" class="absolute bottom-8 right-8 w-[fit-content] rounded-[8px]"
        @click="submit()" />
    </div>
  </div>
</template>
...
.inputField[type="text"] {
  @apply outline-none w-[20rem] h-[2rem] box-content transition duration-100 ease rounded-4px font-medium text-12px border-none px-6px py-2 placeholder-light-400;
  @apply max-h-[90vh];
...
.addNewImageInputField[type="text"] {
  @apply outline-none h-[2rem] w-[20rem] box-border transition duration-100 ease rounded-4px font-medium text-12px border-none px-6px py-2 resize-none;
  background-color: rgba(28, 27, 34, var(--tw-bg-opacity));
  font-family: "Work Sans", sans-serif;
